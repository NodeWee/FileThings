...
async function install(task) {
    ...
    if (platform === "macos") {
        ...
        const suffix_num_arr = ["001", "002"];
        const file_name = "pandoc-3.2-windows-x86_64.zip";
        ...
        await utils.clear_specified_files(exe_dir, "(?i)^pandoc.*$");
        props.progress_current += 5;
        ...
        const result_zip_file = await methods
        ...
        const to_dir = await utils.join_path([exe_dir, "pandoc"]);
        ...
        await utils
            .clear_specified_files(download_dir, "(?i)^pandoc.*$")
            .catch((err) => {
                console.error(err);
                throw new Error(
